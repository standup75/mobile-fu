...
I've added a check, so that if the :mobile.erb template does not exist, the format would fall back to :html
All the mojic is done via:

    lookup_context.find("#{params[:action]}.mobile.erb", params[:controller])
